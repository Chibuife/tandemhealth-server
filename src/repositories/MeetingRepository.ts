...
// ✅ Fixed: patient_id → patient_id, doctor_id → doctor_id (correct column names)
const JOINED_SELECT = `
  SELECT
    m.*,
    p.name  AS patient_name,
...
export const MeetingRepository = {
  async create(input: ScheduleMeetingInput): Promise<Meeting> {
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      try {
        const result = await pool.query(
          `INSERT INTO meetings
            (slug, title, patient_id, doctor_id, scheduled_start, scheduled_end,
             reason_for_visit, priority, consultation_type)
           VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
           RETURNING *`,
          [
            ...
            input.title,
            input.patientId,
            input.doctorId,       // ✅ was input.doctorsId
            input.scheduledStart,
            input.scheduledEnd,
            input.reasonForVisit ?? null,
            input.priority ?? "medium",
  ...
  async findBySlug(slug: string): Promise<Meeting | null> {
    const result = await pool.query(`SELECT * FROM meetings WHERE slug = $1`, [slug]);
    if (result.rowCount === 0) return null;
    return mapRowToMeeting(result.rows[0]);
  },
  ...
  async listForDoctor(
    ...
    let query = `${JOINED_SELECT} WHERE m.doctor_id = $1`;  // ✅ was m.host_id
